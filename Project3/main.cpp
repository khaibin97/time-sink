...
void intTests()
{
  ...
  //new tests
	checkPassFail(intScan(&index,0,GTE,5000,LT), 4999)//test performance
	//rigorous random testing for "specific" tuple
	for(int i=0; i<10000; i++){
		int randomNum = random() % (relationSize-1);
		checkPassFail(intScan(&index,randomNum,GTE,randomNum,LTE),1)
	}
}
