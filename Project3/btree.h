...
class BTreeIndex {
 ...
 bool rootNode; // easy variable to check for root status
 ...
 public:
  ...
  /**
   * Look recursively for entry point
   * Split if full
   * @param cPageNo            Current Page number .
   * @param entry              RIDKeyPair to insert.
   * @param newChildEntry	     PageKeyPair for child if split
   **/
	const void lookup(PageId cPageNo, RIDKeyPair<int> entry, PageKeyPair<int>& newChildEntry);


  /**
   * insert function for leaf node
	 * Split if full
   * @param entry              RIDKeyPair to insert
   **/
	const void insertAtRoot(RIDKeyPair<int> entry);


 /**
  * Split the leaf node
	* Insert the entry after the split.
  * @param entry		        RIDKeyPair to insert.
  * @param lNode            node to pslit.
  * @param nPage	          page to use for parent.
  **/
	const void splitLeaf(RIDKeyPair<int> entry, LeafNodeInt * lNode, PageKeyPair<int> & nPage);


 /**
   * Split the non leaf node
	 * Insert the entry after the split.
   * @param entry                  PageKeyPair to insert.
   * @param nlNode                 non leaf node to split.
   * @param newInsertedPage        page to use for parent.
   **/
	const void splitNonLeaf(PageKeyPair<int> entry, NonLeafNodeInt * nlNode, PageKeyPair<int> & newInsertedPage);


 /**
  * Find the index to insert. 
	* Insert the entry.
  * @param leafnode                Leaf node to insert.
  * @param entry	                 RIDKeyPair entry to insert.
  **/
	void insertAtLeaf(LeafNodeInt* leafNode, RIDKeyPair<int> entry);

 /**
  * Find the index to insert. 
	* Insert the entry.
  * @param nonleafnode             Non leaf node to insert.
  * @param entry	                 RIDKeyPair entry to insert.
  **/
  void insertAtNonLeaf(NonLeafNodeInt* nonLeafNode, PageKeyPair<int> entry);
  ...
  const void findStartRecordID(PageId rootNo);
  const void findKeyInLeaf(PageId leaf);
